...
import frc.robot.commands.Autonomous.Auto_Cross_Line_And_Back;
...
public class RobotContainer {
  ...
  private RobotContainer() {
    ...
    // m_chooser.setDefaultOption("Auto Balance Basic", new Auto_Balance_Basic(m_drivetrain,
    // m_Led_Controller));

    // m_chooser.setDefaultOption("Drive forwards", new DT_AutoArcadeDrive(m_drivetrain, 0, 1, 0.2,
    // 3));
    m_chooser.setDefaultOption(
        "Drive back and forwards", new Auto_Cross_Line_And_Back(m_drivetrain));
    // m_chooser.addOption("Auto Balance", new Auto_Balance(m_drivetrain, m_Led_Controller,
    // m_intake, m_Arm));
    // m_chooser.addOption("Auto Balance Basic", new Auto_Balance_Basic(m_drivetrain,
    // m_Led_Controller));
    m_drivetrain.setDefaultCommand(
        new DT_ArcadeDrive(
            ...
            () -> joystick1.getX(),
            () -> joystick1.getY(),
            () -> joystick1.getThrottle(),
            () -> joystick1.getTrigger()));
    m_elevator.setDefaultCommand(
        new Elevator_Analog_Control(m_elevator, () -> xboxController.getRightY()));
  ...
  private void configureButtonBindings() {
    ...
    final JoystickButton button_A =
        new JoystickButton(xboxController, XboxController.Button.kA.value);
    final JoystickButton button_B =
        new JoystickButton(xboxController, XboxController.Button.kB.value);
    final JoystickButton button_X =
        new JoystickButton(xboxController, XboxController.Button.kX.value);
    final JoystickButton button_Y =
        new JoystickButton(xboxController, XboxController.Button.kY.value);

    final JoystickButton button_LB =
        new JoystickButton(xboxController, XboxController.Button.kLeftBumper.value);
    final JoystickButton button_RB =
        new JoystickButton(xboxController, XboxController.Button.kRightBumper.value);

    // final Trigger rt = new Trigger(() -> xbox.getRightTriggerAxis() > 0.5);
    // final Trigger lt = new Trigger(() -> xbox.getLeftTriggerAxis() > 0.5);
    ...
    // final DPadButton xbox_dpad_Right = new DPadButton(xboxController,
    ...
    button_X.onTrue(
        new LED_Set_Colour_Mode(m_Led_Controller, Constants.BLINKIN_COLOUR_WAVE_RAINBOW));
