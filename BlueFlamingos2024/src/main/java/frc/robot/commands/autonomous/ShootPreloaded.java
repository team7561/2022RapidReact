...
import frc.robot.commands.conveyor.CO_GoUp;
...
public class ShootPreloaded extends SequentialCommandGroup {
  ...
  public ShootPreloaded(Shooter shooter, Conveyor conveyor, Drivetrain drivetrain) {
    addCommands(
      ...
      new ParallelCommandGroup(new TimerCommand(1), new CO_GoUp(conveyor), new SH_Shoot(shooter)),
      new SH_Stop(shooter),
      new DT_AutoArcadeDrive(drivetrain, -1, 0, 0.2, 3)
    );
  }
