...
public class DT_WheelSwerveDrive extends CommandBase {
  @SuppressWarnings({"PMD.UnusedPrivateField", "PMD.SingularField"})
  private final Drivetrain m_subsystem;
  private DoubleSupplier m_x, m_y, m_z, m_twist, m_speed;
  private double target_angle, m_power;

  public DT_WheelSwerveDrive(Drivetrain drivetrain, DoubleSupplier x, DoubleSupplier y, DoubleSupplier twist, DoubleSupplier speed) {
  ...
  public void initialize() {
    m_subsystem.setMode(SwerveMode.CAR);
  }

  @Override
  public void execute() {
    m_power = -m_y.getAsDouble()+m_z.getAsDouble();
    target_angle = m_x.getAsDouble()/6*Math.PI + Math.PI/2;
